...
class HandDetector:
    ...
    def __init__(self, image_mode=False, max_num_hands=4, min_detection_confidence=0.5, min_tracking_confidence=0.5):
        ...
        self.hands = self.mphands.Hands(
            self.image_mode, self.max_num_hands, self.min_detection_confidence, self.min_tracking_confidence)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if len(landmark_list) == 0:
            cv2.putText(img, 'NOT Detected', (10, 140),
            ...
            if (my_fingers[1] and not my_fingers[2]):
                cv2.circle(img, center=(landmark_list[8][1], landmark_list[8][2]), radius=20, color=(
                    255, 255, 0), thickness=-1)
                cv2.putText(img, 'Detected write mode', (10, 140),
                            fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=2, color=(255, 0, 0))
            else:
                cv2.putText(img, 'Detected', (10, 140),
                            fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=2, color=(255, 0, 0))

        currentT = time.time()
        fps = 1/(currentT - previousT)
        previousT = currentT
        cv2.putText(img, 'Client FPS:' + str(int(fps)), (10, 70),
                    fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=2, color=(255, 0, 0))
        cv2.imshow('img', img)
        cv2.waitKey(1)
