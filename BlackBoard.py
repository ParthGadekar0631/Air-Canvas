...
while True:
    ...
    if(len(landmark_list) != 0):   # hand detected
        ...
        # both middle and index fingure are straight
        if (my_fingers[1] and my_fingers[2]):
            ...
            cv2.putText(frame, 'Brush Selection Mode', (900, 680), fontFace=cv2.FONT_HERSHEY_COMPLEX, color=(
                0, 255, 255), thickness=2, fontScale=1)
            cv2.line(frame, (x1, y1), (x2, y2), color=DrawColor, thickness=3)

        # only the index fingure is straight
        elif (my_fingers[1] and not my_fingers[2]):

            cv2.putText(frame, "Writing Mode", (900, 680), fontFace=cv2.FONT_HERSHEY_COMPLEX, color=(
                255, 255, 0), thickness=2, fontScale=1)
            ...
            if DrawColor == (0, 0, 0):
                cv2.line(Board, (xp, yp), (x1, y1),
                         color=DrawColor, thickness=150)

            else:
                cv2.line(Board, (xp, yp), (x1, y1),
                         color=DrawColor, thickness=10)
    ...
    cv2.putText(frame, 'Current Cam FPS:' + str(int(fps)), (10, 670),
